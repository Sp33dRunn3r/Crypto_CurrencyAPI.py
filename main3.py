...
def binance_price3():
    ...
    # LiteCoin to Bitcoin
    candles = client.get_klines(symbol='LTCBTC', interval=Client.KLINE_INTERVAL_1MINUTE)
    ...
    final_data_frame.to_csv('Binance_report3.csv')
    data = pd.read_csv('Binance_report3.csv')
    data['Daily_return_log'] = np.log(data['Close'] / data['Close'].shift(1))
    data.to_csv('final_report3.csv', mode='a', header=True)
    print(data)
    return final_data_frame

print(binance_price3())
